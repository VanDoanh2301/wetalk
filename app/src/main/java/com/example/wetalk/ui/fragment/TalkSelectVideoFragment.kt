...
import com.example.wetalk.ui.adapter.TalkChooseVideoAdapter
 ...
 */
class TalkSelectVideoFragment : BaseFragment() {
    ...
    private lateinit var talkChooseVideoAdapter: TalkChooseVideoAdapter
    fun getPaths(): ArrayList<String> {
        return paths
    }
    companion object {
        fun newInstance(): TalkSelectVideoFragment {
            val args = Bundle()
            val fragment = TalkSelectVideoFragment()
            fragment.arguments = args
            return fragment
        }
    }
    fun setVideoTask(videoTask: Task<ArrayList<StorageImageItem>>): TalkSelectVideoFragment{
    ...
    private fun initVideo() {
        ...
        talkChooseVideoAdapter = object : TalkChooseVideoAdapter (requireContext(), image) {
            override fun OnItemClick(i: Int, path: String?) {
                ...
                talkChooseVideoAdapter.notifyData(paths)
                notifyItemChanged(i)
            }
        }
        rcvImage.adapter = talkChooseVideoAdapter
        val rcvVideo: RecyclerView = binding.rcvVideo
        rcvVideo.layoutManager = GridLayoutManager(activity, 4)
        rcvVideo.adapter = object : TalkChooseVideoAdapter(requireActivity(), video) {
            override fun OnItemClick(i: Int, path: String?) {
                if (paths.contains(path)) {
                    paths.remove(path)
                } else if (paths.size > 4) {
